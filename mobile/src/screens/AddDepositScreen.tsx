...
export function AddDepositScreen({ onAdded, onCancel }: Props) {
  ...
  const handleAdd = async () => {
    ...
    if (!name.trim() || !bank.trim() || isNaN(amt) || amt <= 0 || isNaN(rate) || rate < 0) {
    ...
    try {
      ...
      await portfolioAPI.addDeposit(data);
      onAdded();
    } catch (e) {
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      <View style={styles.form}>
        <Text style={styles.title}>Добавить депозит</Text>
        {error ? <Text style={styles.error}>{error}</Text> : null}

        <Text style={styles.label}>Название</Text>
        <TextInput style={styles.input} placeholder="Вклад накопления" placeholderTextColor="#888" value={name} onChangeText={setName} />

        <Text style={styles.label}>Банк</Text>
        <TextInput style={styles.input} placeholder="Сбербанк" placeholderTextColor="#888" value={bank} onChangeText={setBank} />

        <Text style={styles.label}>Сумма (₽)</Text>
        <TextInput style={styles.input} placeholder="1000000" placeholderTextColor="#888" value={amount} onChangeText={setAmount} keyboardType="decimal-pad" />

        <Text style={styles.label}>Ставка % годовых</Text>
        <TextInput style={styles.input} placeholder="18" placeholderTextColor="#888" value={interestRate} onChangeText={setInterestRate} keyboardType="decimal-pad" />
